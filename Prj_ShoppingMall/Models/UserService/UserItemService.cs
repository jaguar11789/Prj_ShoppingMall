...
namespace Prj_ShoppingMall.Models.UserService
{
    public class UserItemService
    {
        ...
        public CheckInfo purchaseItem(UserInfo objUserInfo, ItemInfo objItemInfo ,string strItemColor, string strItemSize, long lngCpCode, string strReceiveUserId)
        {
            ...
            using (SqlConnection conn = new SqlConnection(strConnectionString))
            {
                using (SqlCommand cmd = new SqlCommand("UP_PURCHASE_TX_INS", conn))
                {
                    ...
                    {
                        Log.Info("[UserItemService][purchaseItem] ex : " + ex);
        ...
        // 회원 아이디 확인(선물)
        public CheckInfo checkReceiveUserId(string strReceiveUserId)
        {
            int intRetVal    = -1;
            string strRetMsg = null;
            CheckInfo objCheckInfo = null;
            ...
            using (SqlConnection conn = new SqlConnection(strConnectionString))
            {
                using (SqlCommand cmd = new SqlCommand("UP_RECEIVEUSERID_TX_CHK", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.Add("@pi_strUserId", SqlDbType.VarChar).Value = strReceiveUserId;
                    ...
                    {
                        Log.Info("[UserItemService][checkReceiveUserId] ex : " + ex);
        ...
        // 상품 구매(선물)
        public CheckInfo purchaseGift(UserInfo objUserInfo, ItemInfo objItemInfo, string strItemColor, string strItemSize, long lngCpCode, string strReceiveUserId)
        {
            int intRetVal = -1;
            string strRetMsg = null;
            CheckInfo objCheckInfo = null;
            ...
            using (SqlConnection conn = new SqlConnection(strConnectionString))
            {
                using (SqlCommand cmd = new SqlCommand("UP_PURCHASEGIFT_TX_INS", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
